 ...
 *
 * Copyright (C) 2006, 2007, 2008 Ronald Huizer
 *
...
{									\
	if (v->sa == 0) {						\
		...
		if (UINT_MAX / VECTOR_GROW_SCALE			\
		             / sizeof( type ) < v->sa) {		\
			warning("vector.c: multiplication overflow; "	\
				"vector_grow() failed!\n");		\
		 ...
		 */							\
			...
			v->sa *= VECTOR_GROW_SCALE;			\
									...
									\
	v->data = (type *)						\
		xrealloc(v->data, v->sa * sizeof( type ));		\
	return v;							\
}
...
{									\
	...
	if (alloc_size < size) {					\
		warning("vector.c: pow overflow; "			\
		        "vector_resize() failed!\n");			\
		return NULL;						\
	}								\
									\
...
{									\
	...
	if (new < v->end || new < s) {					\
		warning("vector.c: addition overflow; "			\
			"vector_add() failed!\n");			\
		return NULL;						\
	}								\
									\
